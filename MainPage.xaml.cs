...
{public sealed partial class MainPage : Page
    {
        ...
        public MainPage()
        {
            this.InitializeComponent(); ResultsListView.ItemsSource = App.ItemsCache;
        ...
        private void ApplyStyle()
        {
            ...
            if (viewStyle == 1) ResultsListView.ItemTemplate = (DataTemplate)this.Resources["GridTemplate"];
            else ResultsListView.ItemTemplate = (DataTemplate)this.Resources["ListTemplate"];
        }
        private async Task LoadAvitoDataAsync()
        {
            ...
            try
            {
                ...
                using (var client = new HttpClient(filter))
                {
                    ...
                    string jsonText; using (var dr = DataReader.FromBuffer(buffer))
                    {
                        dr.UnicodeEncoding = UnicodeEncoding.Utf8;
                        jsonText = dr.ReadString(buffer.Length);
                    }
                    ...
                    var items = json["items"]
                             ...
                             ?? json["result"]?["cards"]
                             ?? json["data"];
                    if (items == null) return;  App.ItemsCache.Clear();
                    foreach (var item in items)
                    {
                        ...
                        if (imgs != null && imgs.HasValues)
                        {
                            ...
                            {
                                imgUrl = rawUrl.Replace("\\", "").Trim(' ', '"', '{', '}', ','); if (imgUrl.StartsWith("//")) imgUrl = "https:" + imgUrl;
